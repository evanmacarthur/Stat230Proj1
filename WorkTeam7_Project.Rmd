...
```{r ggpairs, message=FALSE, fig.width=8, fig.height=5, fig.align='center', echo=FALSE}
ggpairs(df1, columns = 1:7, progress = FALSE)
```
> There are significant correlations with all variables, but lots of these associations do not look linear. Transforming the data may help improve these relationships.
...
> The final model suggested by best subsets regression is the 4-predictor model with pop_dens_2, grazing_rotation, fertilizer_use_2, and farm_acres_2 without bee colonies 2. This model had the lowest $C_p$ score of 4.2828. This model was also recommended by stepwise regression and backwards elimination, with the lowest AIC score of -6545.7. However, since the 5-predictor model including bee_colonies_2 had similar AIC, and $C_p$ scores, we will manually compare the model strength and significance.
...
msummary(fmA2)
``` 
>Transforming our variables resulting in reducing our model significance, as farm_acres is significant in the untransformed model, with farm_acres_2 is not significant in the second model. However, the improvement in meeting conditions seems sufficient to justify continuing with one of the transformed models. The transformed model also has a higher $R^2_adj$ by 6.8%, which means that the transformations also improved model strength. 
...
```{r wrangling good ag data, eval=FALSE, include=TRUE}
df0 <- read_csv("gooddata.csv")
df0 <- df0 |> 
  clean_names()
df0 <- df0 |> 
...
```{r model checking for untransformed with everything accept bees, fig.width=7, fig.keep='last'}
...
```{r conditions for full and full-bees, fig.width=7, fig.height=5, fig.keep='last'}
